...
from ReadRinexNav import ReadOrgUMT
    ...
    for t, tow in enumerate(epoch[length.index(min(length))]):
        ...
        for i in range(0, num_car):
            ...
            ## Add range measurements to PR here
                ...
                dr.append(math.sqrt((float(pos[0]) - float(pos[(i+1)*14])) ** 2\
                                   +(float(pos[4]) - float(pos[4+(i+1)*14])) ** 2\
                                   +(float(pos[8]) - float(pos[8+(i+1)*14])) ** 2))
                Gr_1.append((float(pos[0]) - float(pos[(i+1)*14]))/dr[i])
                Gr_1.append((float(pos[4]) - float(pos[4+(i+1)*14]))/dr[i])
                Gr_1.append((float(pos[8]) - float(pos[8+(i+1)*14]))/dr[i])
                Gr_1.append(0)
                Gr_1 = mat(Gr_1)
                Gr[i, 0:14:4] = Gr_1
        ...
        [pos,P] = Extended_KF(num_car, num_sat, Q, R, P, PR, pos, X, Y, Z, Gr, dr);
...
def Extended_KF(num_car, num_sat, Q, R, P, PR, pos, X, Y, Z, Gr, dr):
    Ii = np.eye(len(pos))
    Ii = mat(Ii)
    ...
    for i in range(0, num_car):
        ...
        ObservationM(pos[14*i:14*(i+1)], X[num_sat[i]:num_sat[i+1]],Y[num_sat[i]:num_sat[i+1]], Z[num_sat[i]:num_sat[i+1]])
...
if __name__ == '__main__':
   ...
   ############### orignal umt ############
        car = ReadOrgUMT("./meas/Car/circle.umt")
