...
def ReadOrgUMT(filename):
    
    car_values = np.loadtxt(filename, delimiter=',', usecols= range(3,27))
    car_t = np.loadtxt(filename, delimiter=',', usecols= 0)

    te = []
    sel = []
    for i,k in enumerate (car_t):
        dgt, intg = math.modf(k)
        if dgt == 0:
            dig, hr = math.modf(k/3600)
            sec, minute = math.modf(dig * 60)
            sec = sec * 60
            utc = [2019, 7, 3, hr, minute, sec]
            c0, c1 = Date2GPSTime(utc)
            te.append([c0,c1])
            sel.append(i)
    
    te = mat(te)        
    car = np.hstack((te, car_values[sel,:]))
