...
function [Xs, Ys, Zs, Xv, Yv, Zv] = UCMP_generateSatTraj_ECEF(startTime, sampTime, dur)
%https://de.mathworks.com/matlabcentral/fileexchange/65845-satellite-constellation
...
for m=1:length(numofsat)
    ...
    %t= endTime; % 24*60*60;                          % time span (24 hrs)
    J= startTime:sampTime:(startTime + dur);% 1:600:t;                           % 10 minutes
    xs=zeros(1,72);
    ys=zeros(1,72);
    zs=zeros(1,72);
    for k=1:length(J)
...
% Satellite velocities
 Xv = diff(Xs,1,2)/sampTime;
 Yv = diff(Ys,1,2)/sampTime;
 Zv = diff(Zs,1,2)/sampTime;
