...
while (~endOfHeader)
    ...
    elseif strfind(line,'DATE')
        ...
        DOY=datenum([real(year),real(month),real(day),0,0,0])-([real(year),1,zeros(1,4)]);
    ...
    if ischar(constellation) %New Ephemeris
        switch constellation
            ...
            case 'R' %Also SBAS case
                ...
                if constellation == 'R'
                    glonass = [glonass, cartesianArray];
...
% Construct output
outputEphemeris.glonass        = real(glonass)';
outputEphemeris.gps            = real(gps)';
outputEphemeris.beidou         = real(beidou)';
...
fclose(navFile);
fprintf('Ephemeris loaded successfully\n\n');
