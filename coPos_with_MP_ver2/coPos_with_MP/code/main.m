...
UMTfolder = '../data/square/umt'
% list of vehicles to use in simulation
 vehiclesToImport = [0] % to use all 15 vehicles! 
...
for v = 1:1:noV % Loop over all vehicles
    ...
    % Undersample the vehicle trajectory   us-undersample 
    ...
    antLat_init_deg = vehTraj(v).LLA(1,1); % degrees, lambda LAT
    antLon_init_deg = vehTraj(v).LLA(2,1); % degrees, phi    LONG
    
    % Computed settings:
    
    antPos_init_ENU = vehTraj_us(:,1)     % ECEF
    antPos_init_LLA = vehTraj(v).LLA(:,1)    % LLA
 ...
 [satPos_ECEF_X, satPos_ECEF_Y, satPos_ECEF_Z,...
    ...
    ephemeris, XYZ_station);                              % SAT_POS,VEL AT unsampled rate
